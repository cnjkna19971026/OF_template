...
#def get_regions_from_mesh(case_dir):
#    fluids = []
#    solids = []
#
#    constant_path = os.path.join(case_dir, "constant")
#    
#    if not os.path.exists(constant_path): return None, None
#
...
#            if "air" in item.lower() or "fluid" in item.lower():
#                fluids.append(item)
#            else:
#                solids.append(item)
...
def fix_boundary_types(case_dir, regions, log_func):
    ...
    for region in regions:
        ...
        if not os.path.exists(b_file):
            log_func(f"[Warning] No boundary file for region '{region}' ")
            continue
